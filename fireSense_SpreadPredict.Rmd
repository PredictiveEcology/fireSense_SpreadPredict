...
# keep R packages isolated in their own folder -- can omit this to use existing packages (may not work correctly)
# origRPath <- setLibPaths("tmpFireSenseRPackages/")   

# See ?install_github -- Just install whatever is available; don't ask for further updates -- this prevents prompts to update
Sys.setenv("R_REMOTES_UPGRADE" = "never") 

Require(c("magrittr", "raster", "SpaDES.core", "SpaDES.tools"))
Require(c("logging", "RandomFields")) # These are suggested from SpaDES.core and used
...
Run the model: <!-- TODO: This doesn't have data, so doesn't run -->
...
```{r reset-R-package-library}
if (exists("origRPath")) setLibPaths(origRPath)   
```
