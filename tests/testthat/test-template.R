...
# 2. Copy this file to the tests folder (i.e., ` # replace with empty string insteadfireSense_SpreadPredict/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("fireSense_SpreadPredict")
  ...
  parameters <- list(
    ...
    fireSense_SpreadPredict = list(.saveInitialTime = NA)
  )
  ...
  if (exists("fireSense_SpreadPredictEvent1", envir = .GlobalEnv)) {
    simOutput <- fireSense_SpreadPredictEvent1(mySim)
  } else {
    simOutput <- mySim$fireSense_SpreadPredictEvent1(mySim)
  }
  ...
  if (exists("fireSense_SpreadPredictEvent2", envir = .GlobalEnv)) {
    simOutput <- fireSense_SpreadPredictEvent2(mySim)
  } else {
    simOutput <- mySim$fireSense_SpreadPredictEvent2(mySim)
  }
